...
def leaveTopK(preds,K):
    _,idx = torch.sort(preds[:,-3],descending=True)
    preds = preds[idx,:]
    ...
    for i in range(1,min(K,preds.size()[0])):
        ...
        if t[:,-3] == 0:
            break
        m,_ = torch.max(F.cosine_similarity(t[:,:-3].repeat(out.size()[0],1),out[:,:-3],dim=1),dim=0)
        if m<1:
...
#     print(preds_r1)
    ...
    if non_zero.size()[0]==0:
        ...
        #predicate of body2 with predicate of rule
        preds_r3[non_zero] = preds_r3[non_zero]*F.cosine_similarity(rule_expanded[non_zero,2*num_predicates:],
                                                                    body2[non_zero,:num_predicates],dim=1)
    if non_zero.size()[0]==0:
        preds_r3 = torch.zeros_like(preds_r3)
    else:
...
with open('s2_auc-pr_masking','w') as f:
    # for lamb in lambdas:
    suc_rate_neigh = 0
    suc_rate_locin = 0
    accuracies = []
    for _ in range(10):
        ...
        for epoch in range(num_iters):
            for par in optimizer.param_groups:
                par['params'][2].data.clamp_(min=0.1,max=0.9)
